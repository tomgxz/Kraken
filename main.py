class Kraken():
    ...
    def generateNewSite(self,siteName):
        self.logger.info(f"Generating a new site for user {self.username}")

        sitePath=f"data\\user\\{self.username}\\sites\\{siteName}"
        ...
        siteConfigFileContent=self.configparser()

        siteConfigFileContent.read(siteConfigFile)

        section="site"
        siteConfigFileContent.add_section(section)
