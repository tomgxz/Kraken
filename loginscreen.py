...
class LoginScreen(Screen):
    ...
    def onSignupOptionClickEvent(self,e):
        from signupscreen import SignupScreen
        SignupScreen(self.master,self.root)
        self.clear()
        
    def onPasswordShowClickEvent(self,e):
        self.passwordShow=self.tkinter.Entry(self.passwordContainer,textvariable=self.loginScreenPasswordTextVar,width=32,fg=self.loginScreenPasswordEntry["fg"],relief="solid")
        self.passwordShow.grid(row=0,column=1)

    def onPasswordShowUnclickEvent(self,e):
